...
export class TrainingBetController {
    constructor(
        private readonly trainingBetService: TrainingBetService
    ) { }

    @Post()
    async create(@Body() object: CreateTrainingBetDto): Promise<TrainingBetEntity> {
        try {
    ...
    async update(
        ...
        try {
            ...
            new Exception({ ...e, message: `Falha ao atualizar aposta: ${e.message}` });
        }
    }
    ...
    @Get()
    async findAll(@Query() options: FindOptionsDto<TrainingBetEntity>): Promise<FindReturnModelDto<TrainingBetEntity>> {
        try {
